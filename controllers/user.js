...
// The controller function for singup
async function controllerSignup(req, res){

    // check if the body includes username and password
    if (!req.body.username || !req.body.password){
        return res.status(400).end("<p>Not created</p>");
    }

    // exract username and password from the body
    const {username, password} = req.body;
    //hash the password and store in the variable
    ...
    if(!newUser){
        return res.status(500).end("<p>Not created</p>");
    }
    // if user is created successfully, this function will run:
...
module.exports = {controllerSignup, controllerSignin};
